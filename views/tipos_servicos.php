<h3 style="background-color:#286090;color:#FFFFFF;padding:7px;">Cadastro de tipos de serviços</h3>
<div class="row">
    <div class="col-sm-12">
                    ...
                    <div class="modal-body">
                        <form method="POST">
                            <div class="form-group">
                                <p>Novo registro de Tipos de serviços</p>
                                <label for="nome">Descrição:</label>
                                <input type="text" name="descricao" class="form-control descricao" style="text-transform:uppercase;" />
                                <label for="valor">Valor:</label>
                                <input type="text" name="valor" class="form-control" style="text-transform:uppercase;" onkeypress="return SomenteValor(this, event)" maxlength="10" />
        ...
        <div class="tabelario" style="display:none;">
            <table id="tbItems" class="display" cellspacing="0" width="100%" style="display:none;">
                <thead>
                    ...
                    <td class="header_treat">Tipos de serviços:</td>
                ...
                <tbody>
                    <?php foreach($tipos_servicos as $tser): ?>
                        <tr>
                            <td style="width:2%;text-align:center;"><?php echo $tser['pk_ser']; ?></td>
                            <td><?php echo $tser['descricao']; ?></td>
                            <td><?php echo "R$" . number_format($tser['valor'], 2, ",", "."); ?></td>
                            <?php if($tser['inativo'] == "S"): ?>
                                <td style="width:2%;text-align:center;background-color:red;color:white;">Sim</td>
                            <?php else: ?>
                                <td style="width:2%;text-align:center;">Não</td>
                            <?php endif; ?>
                            <td style="text-align:right">
                                <a href="#" data-toggle="modal" data-target="#editarRegistro<?php echo $tser['pk_ser']; ?>" title="Editar registro" style="text-decoration:none;color:green;cursor:pointer;"><span class="glyphicon glyphicon-edit"></span>Editar</a>&nbsp;
                                <a class="" title="Excluir registro" onclick="deletarRegistro(<?php echo $tser['pk_ser']; ?>)" style="text-decoration:none;color:red;cursor:pointer;"><span class="glyphicon glyphicon-remove"></span>Excluir</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <?php foreach($tipos_servicos as $tser): ?>
        <!-- MODAL EDITAR INICIO -->
        <div id="editarRegistro<?php echo $tser['pk_ser']; ?>" class="modal fade bd-example-modal-sm editarRegistro" role="dialog" aria-labelledby="EditarRegistro" data-backdrop="static">
            <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                    <div class="modal-body">
                        <form method="POST" id="registroEditar<?php echo $tser['pk_ser']; ?>">
                            <div class="form-group">
                                <p>Editar registro de Tipos de serviços</p>
                                <label for="nome">Descrição:</label>
                                <input type="text" name="descricao_editar" class="form-control descricao_editar" style="text-transform:uppercase;" value="<?php echo $tser['descricao']; ?>" />
                                <label for="valor_editar">Valor:</label>
                                <input type="text" name="valor_editar" class="form-control" style="text-transform:uppercase;" onkeypress="return SomenteValor(this, event)" maxlength="10" value="<?php echo str_replace(".", ",", $tser['valor']); ?>" />
                                <label for="inativo">Inativo:</label>
                                <select name="inativo_editar" id="inativo_editar" ref="inativo" class="form-control" required="required">
                                    <option value="NNN">Selecione</option>
                                    <option value="S" <?php echo $tser['inativo'] == "S" ? 'selected':''; ?>>Sim</option>
                                    <option value="N" <?php echo $tser['inativo'] == "N" ? 'selected':''; ?>>Não</option>
                                </select>
                            </div>
                            <button type="button" class="btn btn-primary" onclick="atualizarRegistro(<?php echo $tser['pk_ser']; ?>)">Atualizar</button>
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button>
                        </form>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function salvarRegistro(){
        ...
        items.append("valor", valor);
        items.append("inativo", inativo);

        axios({
            method: "POST",
            url: "tipos_servicos/novoRegistro",
            data: items
        }).then(res => {
    ...
    //EDITAR REGISTRO
    function atualizarRegistro(pk_ser){
        var descricao = $("#registroEditar" + pk_ser).serializeArray()[0].value;
        var valor = $("#registroEditar" + pk_ser).serializeArray()[1].value;
        var inativo = $("#registroEditar" + pk_ser).serializeArray()[2].value;
        ...
        axios({
            method: "POST",
            url: "tipos_servicos/editarRegistro",
            data: items
        }).then(res =>{
            var data_return = res.data;
            console.log(res);
    ...
    //DELETAR REGISTRO
    function deletarRegistro(pk_ser){
        let items = new URLSearchParams();
        items.append("pk_ser", pk_ser);
        ...
        swal({
            ...
            if (result.value) {
                axios({
                    method: "POST",
                    url: "tipos_servicos/deletarRegistro",
                    data: items
                }).then(res => {
                    var data_return = res.data;
                    console.log(res);
                    if (data_return == "SUCCESS") {
                        ...
                        swal("", "Não é possível excluir este registro! Associados com esta Tipos de serviços.", "error");
                    }
                });
            }
        });
